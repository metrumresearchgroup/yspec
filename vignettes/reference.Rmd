...
- Use double-quotes around anything you want to be a string
- `true` and `yes` by themselves will be rendered as `TRUE`; use `"yes"` if you 
  need that word by itself
- `false` and `no` by themselves will be returned as `FALSE`; use `"no"` if you
  need that word by itself
- If a string starts with a piece of punctuation, make sure to put it in quotes
    - Use `"> QL"` not `> QL`
    - Use `"."` not `.`
    - Use `"123 Main St."` not `123 Main St.`
...
- `short: short-name` 
    - a short name for the column; don't include units here
- `unit: numeric` 
    - the unit of measure
